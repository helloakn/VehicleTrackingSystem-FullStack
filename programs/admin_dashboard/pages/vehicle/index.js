...
function Dashboard({ query }) {
  ...
  useEffect(() => {
    ...
    const _url = publicRuntimeConfig.NEXT_PUBLIC_ApiDomain + "vehicle";
    setLoading(true);
  ...
  if (!data) return(
    <DashboardLayout 
        head={<Header title="Vehicle Tracking System : Admin Dashboard" />}
        pageName={<div><i className="fas fa-car"></i><label>Vehicle : List</label></div>}
        activePage="vehicle"
    >
       <div className={styles.mainContainer}>
        No Data...
...
let tableRow = data.data.map(function(element,index){
  return (<tr key={"tr"+element.id}>
      ...
      <td>{element.gps_model}</td>
      <td>{element.admin_name}</td>
      <td style={{textAlign: 'center'}}>{(new Date(element.created_at)).toLocaleDateString("en-US", options)}</td>
      <td>
        <div className="tdActionContainer">
        <Link href={"vehicle/detail/"+element.id}>
          <a className="btn-light" style={{color:"#60abc4"}}>
       ...
       <div className={styles.mainContainer}>
        <div className={styles.mainInnerContainer}>
          <div className={styles.paginationContainer}>
            <Pagination paginate={paginate} styles={styles} root={"/vehicle/"} page_at={paginate.page_at} />
          </div>
